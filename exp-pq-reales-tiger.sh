...
ruta_query2="$HOME/DATASETS/REALES/QUERY/tiger_query.txt"
#dataset grandes
for kvecino in "${kvecinos[@]}"; do
    echo "Ejecutando pq con k: $kvecino"
   #euclidean
    ./getTime $ruta_dataset"tiger" $ruta_query2 $kvecino 1 0 1 0 >> "$resultados_file"
    #manhattan
    ./getTime $ruta_dataset"tiger" $ruta_query2 $kvecino 1 0 2 0 >> "$resultados_file"
done
